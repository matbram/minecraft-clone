...
function makeKnobs(time: { phase: number }): Knob[] {
  return [
    ...
    {
      label: 'Torch light range',
      desc: 'How far light from a held/placed torch or flare reaches.',
      group: 'Lighting',
      min: 4,
      max: 24,
      step: 1,
      heavy: undefined,
      get: () => Math.round(Tunables.torchRange),
      set: (s) => (Tunables.torchRange = s),
      fmt: (s) => `${s}`,
    },
    {
      label: 'Torch light brightness',
      desc: 'How bright a torch or flare glows.',
      group: 'Lighting',
      min: 0,
      max: 200,
      step: 10,
      heavy: undefined,
      get: () => Math.round(Tunables.torchIntensity * 100),
      set: (s) => (Tunables.torchIntensity = s / 100),
      fmt: (s) => `${s}%`,
    },
    {
